...
const FAVORITES_STORAGE_KEY = 'gasolineraFavoritos';
...
const MainContent = React.memo(({
    ...
    debouncedDistanceKm, geoError, averagePrice, viewMode, setViewMode, calculateDistance
}) => {
...
function App() {
    ...
    const [favoriteStationIds, setFavoriteStationIds] = useState(() => new Set(initializeStateFromLocalStorage(FAVORITES_STORAGE_KEY, [])));
    ...
    // Efecto para guardar los favoritos en localStorage cada vez que cambien
    useEffect(() => {
        try {
    ...
    // Función para añadir/eliminar de favoritos en localStorage
    ...
    const processedStations = useMemo(() => {
        ...
        let sortedStations = currentFilteredStations.filter(station => {
        ...
        sortedStations.sort((a, b) => {
            ...
            let distB = Infinity;
            if (latitude !== null && longitude !== null && !geoError) {
                distA = calculateDistance(latitude, longitude, cleanPrice(a['Latitud'].replace(',', '.')), cleanPrice(a['Longitud (WGS84)'].replace(',', '.')));
                distB = calculateDistance(latitude, longitude, cleanPrice(b['Latitud'].replace(',', '.')), cleanPrice(b['Longitud (WGS84)'].replace(',', '.')));
            }
            let result;
            if (sortBy === 'price') {
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-screen w-full bg-gradient-to-br from-blue-50 to-indigo-100 font-inter p-4">
                <div className="text-xl font-semibold text-blue-700 animate-pulse mb-4">
                    {apiLoading ? 'Cargando datos de gasolineras...' : 'Cargando...'}
                </div>
